...
const char file_stream = 'f'; // Indicates file stream
...
ifstream ifs {""}; // Declare input file stream
ofstream ofs {""}; // Declare output file stream
...
Token Token_stream::get() // Get characters from cin
{
	...
	if (ofs) {
		if (ch == ';') ofs << ' ';
		else if (!isdigit(ch) && ch!= file_stream) ofs << ch;
	}
	if (isspace(ch)) return Token(prompt); // Return prompt if it is a new line
	switch (ch) {
	case '(':
	case ')':
	case '+':
	...
	default:
		if (isalpha(ch)) { // If it is a letter
			...
			if (s == "from" || s == "to") {
				string b;
				while(cin >> ch && ch!=';') b+=ch; // Continue to read in 's'
				if (s == "from") {
					ifs.close();
					ifs.open(b, ios_base::in);
					if (!ifs) error("Wrong file name");
				}
				if (s == "to") {
					ofs.close();
					ofs.open(b, ios_base::out);
					if (!ofs) error("Wrong file name");
				}
				return Token(file_stream);
			}
...
Token_stream ts(cin); // Holds all Tokens

Symbol_table names; // Holds all variables

double expression(Token_stream&); // Declaration of expression()

double primary(Token_stream&); // Declaration of primary()
...
double primary(Token_stream& ts) // Processes semicolons, numbers and returns variables
{
	Token t = ts.get(); // Get a character
	switch (t.kind) {
	case '(': 
	{	double d = expression(ts); // Perform calculations in semicolons
	...
	case '-': // For negative digits
		return - primary(ts); // Return negative digit
	...
	{	
		...
		t = ts.get();
		if (t.kind == assign) names.set_value(s,expression(ts)); // If there is an assignment symbol then update the value of variable
		else ts.unget(t);
...
double term(Token_stream& ts) // Performs '*', '/', '!', '%' and @(sqrt) operations
{	
	double left = primary(ts); // Get a number
	while(true) {
...
double expression(Token_stream& ts) // Performs '+' and '-' operations
{
	double left = term(ts); // Get the number or the result of calculations in term
	while(true) {
		Token t = ts.get();
		switch(t.kind) {
		case '+':
			left += term(ts); // Addition
			break;
		case '-':
			left -= term(ts); // Subtraction
			break;
		default:
			ts.unget(t); // If nothing was done return the character to the stream
...
double Symbol_table::define_name(char type) // Defines a variable
{
	...
	double d = expression(ts); // Process the provided value
	switch(type) {
	case let:
...
{	
	if (ifs) return; // If we found error in file, proceed to the next expression
	ts.ignore(print);
}
...
void calculate() // Performs calculations
{
	while(true) try {
		...
		if (t.kind == file_stream) continue;
		ts.unget(t); // Return a character into the stream
		double computation = statement();
		cout << result << computation << endl; // Output the result
		ofs << result << computation << endl;
	}
