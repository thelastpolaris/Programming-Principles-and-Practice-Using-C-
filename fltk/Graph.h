...
struct Color {
    ...
    Color(unsigned char r, unsigned char g, unsigned char b) :c(Fl_Color(fl_rgb_color(r,g,b))), v(visible) { }
    Color(Transparency vv) :c(Fl_Color()), v(vv) { }    // default color
    Color(int cc, Transparency vv) :c(Fl_Color()), v(vv) { }
...
public:
    ...
    Font(int ff) :f(ff) { }
    //Font operator=(const Font &other) {return Font(other.as_int());}
...
struct Text : Shape {
    ...
    int length() const; // Get a number of characters
    int width() const { return length()*font_size();}
private:
    string lab;    // label
    Font fnt { fl_font() };
...
struct Mark : Marks {
    ...
    void set_point(int n, Point p) {Shape::set_point(n,p);};
};
...
struct Image : Shape {
    ...
    int width() const { return p->w(); }; // Width of an image
    int height() const { return p->h(); }; // Height of an image
private:
