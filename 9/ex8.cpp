...
public:
	...
	string write_name() {return n;};
	int write_cn() {return cn;};
	double write_fee() {return f;};
	void fee(double sum) {f += sum;}; // Changes the value of fee
	bool check_fee() {return f;}; // Checks if the user owes a fee
private:
...
int main() try {
	...
	//cout << book;
	//cout << endl;
	Patron user{"John Howard", 12345, 0};
	user.fee(30);
	cout << user.check_fee();
}
catch (Book::Invalid) {
