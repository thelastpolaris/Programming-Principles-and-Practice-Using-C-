...
Token Token_stream::get() // Get characters from is
{
	if (full) { full=false; return buffer; } // If buffer is not empty, return value from it
	char ch;
	is >> ch; // Otherwise start looking for characters in input stream
	if(ch == '\0') return Token(print);
	...
	case '9':
	{	
		is.unget(); // If it is a number then return it to the stream
		double val;
		is >> val; // Read the complete value
	...
	default:
		if (isalpha(ch)) { // If it is a letter
			...
			is.unget(); // Return the character into the stream
...
void Token_stream::ignore(char c) // Ignores all characters before specific symbol
{
	...
	char ch;
	while (is>>ch) // Read all values before 'c'
...
string calculate() // Performs calculations
{
	 try {
		...
		cout << "Welcome to Awesome Calculator " << version << " ! Here is the list of the commands:\n"
			...
			"'pow(x,y)' - multiplies x by itself y times (y must be of type int)\n"
			"'sqrt(x)' - returns square root of x\n"
			"'Expression' + '!' - calculates factorial\n"
		...
		return to_string(statement()); // Output the result
	}
	}
